...
public class EmergenciaRepositoryImp implements EmergenciaRepository {
    ...
    public Emergencia updateEmergencia(Emergencia emergencia, long id) {
        try(Connection conn = sql2o.open()){
            conn.createQuery("UPDATE voluntario SET nombre = :newNombre, descrip = :newDescrip, finicio = :newFinicio, ffin - :newFfin, institucion = :newInstitucion WHERE id = :updateId")
